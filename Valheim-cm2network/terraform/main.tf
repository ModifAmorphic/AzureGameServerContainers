...
locals {
  env_secrets = {
    SERVER_PW = "${data.azurerm_key_vault_secret.server_pass.value}"
...
resource "azurerm_container_group" "valheim" {
  ...
  container {
    ...
    commands = [
        "/bin/bash", "-c", 
        "cd /home/steam/valheim-dedicated/; ./start_game_bepinex.sh; tail -F nothing"
          #"echo 'Starting Init'; export LD_LIBRARY_PATH='./doorstop_libs:$${LD_LIBRARY_PATH}'; export LD_PRELOAD='libdoorstop_x64.so:$${LD_PRELOAD}'; export LD_LIBRARY_PATH='./linux64:$${LD_LIBRARY_PATH}'; tail -F nothing; echo 'Finished Init. Starting Server'; tini", "-g", "--", "/home/steam/tinientry.sh"
    ]

    volume {
      name       = var.azure_saves_share
      mount_path = "/home/steam/valheim-worlds"
      read_only  = false
    ...
    volume {
      name       = var.azure_server_share
      mount_path = "/home/steam/valheim-dedicated"
      read_only  = false
    ...
    volume {
      name       = var.azure_backups_share
      mount_path = "/home/steam/valheim-backups"
      read_only  = false
